...
class Disperse(KaitaiStruct):
    ...
    class Data(KaitaiStruct):
        ...
        def _read(self):
            self.seg_pos = [None] * (
                ((2 * self._root.header.ndims) * self._root.header.nsegs)
            )
            ...
            self.nodes_pos = [None] * (
                (self._root.header.ndims * self._root.header.nnodes)
            )
            ...
            self.segdata = [None] * (
                (self._root.header.nsegs * self._root.header.nsegdata)
            )
            ...
            self.nodesdata = [None] * (
                (self._root.header.nnodes * self._root.header.nnodedata)
            )
            ...
            for i in range(self._root.header.nnodes):
                self.nodesdata_int[i] = self._root.NodeStruct(
